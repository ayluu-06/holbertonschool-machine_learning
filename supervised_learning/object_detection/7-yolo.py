...
class Yolo:
    ...
    def preprocess_images(self, images):
        ...
        pimgs = np.zeros((ni, in_h, in_w, 3), dtype=np.float32)
        shapes = np.zeros((ni, 2), dtype=np.int32)
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        for (x1, y1, x2, y2), cls, score in zip(boxes_int, box_classes, box_scores):
            x1 = max(0, min(w - 1, x1))
            y1 = max(0, min(h - 1, y1))
            x2 = max(0, min(w - 1, x2))
            y2 = max(0, min(h - 1, y2))

            cv2.rectangle(img, (x1, y1), (x2, y2), color_box, thickness_box)

            class_name = self.class_names[int(cls)] if 0 <= int(cls) < len(self.class_names) else str(int(cls))
            text = "{} {:.2f}".format(class_name, float(score))

            text_x, text_y = x1, max(0, y1 - 5)

            cv2.putText(
                img, text, (text_x, text_y),
                font, font_scale, color_text, thickness_text, line_type
            )
        ...
        if key == ord('s'):
            ...
            out_path = os.path.join("detections", file_name)
        ...
        pimgs, shapes = self.preprocess_images(images)
        raw_outs = self.model.predict(pimgs)
        if not isinstance(raw_outs, (list, tuple)):
            raw_outs = [raw_outs]

        predictions = []
        for i, img in enumerate(images):
            outs_i = [out[i] for out in raw_outs]
            boxes, box_confidences, box_class_probs = self.process_outputs(
                outs_i, shapes[i])
            f_boxes, f_classes, f_scores = self.filter_boxes(boxes, box_confidences, box_class_probs)
            nms_boxes, nms_classes, nms_scores = self.non_max_suppression(
                f_boxes, f_classes, f_scores)
            window_name = os.path.basename(image_paths[i])
            self.show_boxes(
                img, nms_boxes, nms_classes, nms_scores, window_name)
